...
function openDeleteDialog(provider: LLMProvider) {
  selectedProvider.value = provider
...
async function handleTest(provider: LLMProvider) {
  ...
  try {
    const res = await llmAPI.test(provider.id)
    testResult.value = {
      id: provider.id,
      ok: res.ok,
      message: res.ok
        ? `Connected (${res.latency_ms}ms): ${res.model}`
        : `Failed: ${res.error}`,
    }
  } catch (e) {
    testResult.value = {
...
async function handleToggleDefault(provider: LLMProvider) {
  try {
    await llmAPI.update(provider.id, { is_default: !provider.is_default })
...
const columnHelper = createColumnHelper<LLMProvider>()
  ...
  columnHelper.accessor('name', {
    ...
    cell: (info) =>
      h('div', { class: 'flex items-center gap-2' }, [
        h(Brain, { class: 'h-4 w-4 text-muted-foreground' }),
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-muted-foreground">
            Manage AI model providers for paper analysis
          </p>
        </div>
      ...
      <Card>
        <CardContent class="pt-6">
          ...
          <div v-else-if="error" class="flex flex-col items-center justify-center py-12 text-center">
            <Brain class="h-12 w-12 text-destructive/50 mb-4" />
          ...
          <div v-else-if="table.getRowModel().rows.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
            <Brain class="h-12 w-12 text-muted-foreground/50 mb-4" />
            <h3 class="text-lg font-medium">No LLM providers configured</h3>
            <p class="text-sm text-muted-foreground mt-1">
              Add a provider to enable AI-powered paper analysis
            </p>
          </div>
