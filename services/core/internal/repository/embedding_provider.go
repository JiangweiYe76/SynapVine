...
// EmbeddingProviderRepository persists EmbeddingProvider rows in MySQL.
type EmbeddingProviderRepository struct {
	db *sql.DB
}

// NewEmbeddingProviderRepository returns an EmbeddingProviderRepository backed by the given *sql.DB.
func NewEmbeddingProviderRepository(db *sql.DB) *EmbeddingProviderRepository {
	return &EmbeddingProviderRepository{db: db}
}
...
func (r *EmbeddingProviderRepository) Create(ctx context.Context, p *model.EmbeddingProvider) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO embedding_providers (id, name, base_url, api_key, model, dimensions, is_default, is_enabled, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.Name, p.BaseURL, p.APIKey, p.Model, p.Dimensions, p.IsDefault, p.IsEnabled, p.CreatedAt, p.UpdatedAt,
	)
	if err != nil {
...
func (r *EmbeddingProviderRepository) GetByID(ctx context.Context, id string) (*model.EmbeddingProvider, error) {
	...
	return scanEmbeddingProvider(row)
}
...
func scanEmbeddingProvider(row *sql.Row) (*model.EmbeddingProvider, error) {
	var p model.EmbeddingProvider
