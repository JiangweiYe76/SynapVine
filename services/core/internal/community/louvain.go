...
//
// Community IDs are fresh UUIDs generated by the detector; they have no
// relation to any user-supplied IDs. Run-to-run they are not stable.
...
func nameCommunity(memberIDs []string, nodes []model.Node) string {
	// Community naming derives from the first member node so each cluster
	// is recognisable on its own. Grouping is done by the Louvain
	// algorithm over the edge structure, not by any external taxonomy.
